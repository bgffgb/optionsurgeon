...
class Distribution:
    ...
    def get_cumulative(self, x):
        ...
        if self.type == 'FF':
            ...
            ba2 = d1B / 2
            bb2 = d2B / 2
            return r * betainc(ba, bb, bx) + (1 - r) * betainc(ba2, bb2, bx2)
        if self.type == 'N':
            ...
            r = (x - st0) / (st1 - st0)
            estim_prob = (1 - r) * pr0 + r * pr1
            return estim_prob
        if self.type == 'MSGT':
            val, err = quad(modded_sgt, 0, x, args=(self.params[0], self.params[1], self.params[2]))
            val /= self.norm
            return val
            #ind = np.searchsorted(self.interpolated_strikes, x)
            #ind = min(ind, len(self.cumulative) - 1)
            #estim_prob = self.cumulative[ind]
            #return estim_prob
    ...
    def get_prob_arrays(self, steps=200, th_lower=None, th_upper=None, strike_min=None, strike_max=None):        
